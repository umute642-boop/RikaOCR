...
Optional transliteration is a separate post-recognition layer. The recognised
``Document`` and its Arabic-script ``Line.text`` values are left unchanged;
Latin-script output is written separately.
...
def build_pipeline(
    ...
    *,
    ...
    """Construct a recognition pipeline for the chosen engine."""
    if engine == "dummy":
